...
const iconMap: Record<string, React.ElementType> = {
  Users, Monitor, Brain, Globe, Hotel, Factory, Store, Briefcase, ShoppingBag,
};
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16 animate-slide-up">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Choisissez le domaine qui correspond à vos ambitions professionnelles
            </p>
          </div>
            ...
            {(domains || []).map((domain, index) => {
              ...
              return (
                <Link to={`/formations?domaine=${domain.slug}`} key={domain.id}>
                  <Card className="border-primary/10 hover:border-accent/50 transition-all group hover:shadow-2xl backdrop-blur-sm bg-card/80 rounded-3xl animate-scale-in overflow-hidden h-full cursor-pointer" style={{ animationDelay: `${index * 0.1}s` }}>
                    <CardContent className="p-8 text-center relative">
                      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity" 
                           style={{ background: 'var(--gradient-glow)' }} />
                      <div className="relative z-10">
                        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-accent/20 to-secondary/20 mb-4 group-hover:scale-110 transition-transform">
                          <Icon className="text-accent" size={28} />
                        </div>
                        <h3 className="text-lg font-bold mb-2 group-hover:text-accent transition-colors">{domain.name}</h3>
      ...
      <section className="py-20 bg-card relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16 animate-slide-up">
            <div className="inline-block mb-4 px-6 py-2 rounded-full bg-primary/10 text-primary font-medium text-sm">
              🏢 Nos secteurs
            </div>
            <h2 className="text-4xl md:text-5xl font-bold text-foreground mb-4">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Des formations pensées pour répondre aux enjeux spécifiques de votre industrie
            </p>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {(sectors || []).map((sector, index) => {
              ...
              return (
                <div key={sector.id} className="flex items-center gap-3 px-6 py-4 rounded-full bg-gradient-to-r from-primary/5 to-accent/5 border border-primary/10 hover:border-accent/40 hover:shadow-lg transition-all animate-scale-in cursor-default" style={{ animationDelay: `${index * 0.1}s` }}>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 rotate-180">
          <svg viewBox="0 0 1440 120" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full">
            <path d="M0,108L48,109.5C96,111,192,113,288,112C384,111,480,108,576,107C672,106,768,108,864,109.5C960,111,1056,111,1152,109.5C1248,108,1344,106,1392,105.5L1440,105L1440,120L1392,120C1344,120,1248,120,1152,120C1056,120,960,120,864,120C768,120,672,120,576,120C480,120,384,120,288,120C192,120,96,120,48,120L0,120Z" fill="hsl(var(--background))" fillOpacity="1"/>
          </svg>
        </div>
